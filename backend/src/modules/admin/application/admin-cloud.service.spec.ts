...
describe('assertPermission', () => {
  it('leve ForbiddenException si le role na pas la permission', async () => {
  ...
  it('ne leve pas exception si le role a la permission', async () => {
    const service = makeService();
...
describe('listSubscriptions', () => {
  ...
  it('ne applique aucun filtre si les champs sont absents', async () => {
    const service = makeService();
    await service.listSubscriptions(userAdmin, {
      page: 1,
      limit: 20,
...
describe('listDocuments', () => {
  ...
  it('ne applique aucun filtre si les champs sont absents', async () => {
    const service = makeService();
    await service.listDocuments(superAdmin, { page: 1, limit: 20 } as AdminDocumentsQueryDto);
    expect(mockQb.andWhere).not.toHaveBeenCalled();
  });

  it('applique la pagination', async () => {
    const service = makeService();
    await service.listDocuments(superAdmin, { page: 2, limit: 10 } as AdminDocumentsQueryDto);
  ...
  it('leve ForbiddenException pour user_admin sans CLOUD_READ', async () => {
    const service = makeService();
    await expect(
      service.listDocuments(userAdmin, { page: 1, limit: 20 } as AdminDocumentsQueryDto),
    ).rejects.toThrow(ForbiddenException);
  });
});
...
describe('reprocessOcr', () => {
  ...
  it('propage erreur du use-case', async () => {
