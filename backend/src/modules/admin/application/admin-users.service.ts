...
export class AdminUsersService {
  constructor(
    ...
    @InjectRepository(UserSessionEntity) private readonly sessionsRepo: Repository<UserSessionEntity>,
  ) {}
  ...
  async list(actor: { id: string; role: Role; }, query: AdminUsersQueryDto) {
    ...
    if (query.q) {
      ...
      qb.andWhere(new Brackets(b => {
        ...
        if (parts.length >= 2) {
        b.orWhere(
          '(u.firstName ILIKE :first AND u.lastName ILIKE :last)',
          { first: `%${parts[0]}%`, last: `%${parts[parts.length - 1]}%` }
        );
        b.orWhere(
          '(u.firstName ILIKE :last AND u.lastName ILIKE :first)',
          { first: `%${parts[0]}%`, last: `%${parts[parts.length - 1]}%` }
        );
    } else {
      b.orWhere('u.firstName ILIKE :q', { q: `%${query.q}%` })
       .orWhere('u.lastName ILIKE :q', { q: `%${query.q}%` });
    }
  }));
}
    ...
    if (query.emailVerified !== undefined) qb.andWhere('u.emailVerified = :ev', { ev: query.emailVerified });
    if (query.mfaEnabled !== undefined) qb.andWhere('u.mfaEnabled = :mfa', { mfa: query.mfaEnabled });
  ...
  async ban(actor: { id: string; role: Role; }, userId: string, reason?: string, meta?: any) {
...
async unban(actor: { id: string; role: Role; }, userId: string, meta?: any) {
  const user = await this.mustGetUser(userId);
...
async verifyEmail(actor: { id: string; role: Role; }, userId: string, meta?: any) {
  const user = await this.mustGetUser(userId);
  assertCanActOnUser({ actorRole: actor.role, targetRole: user.role_key, action: 'verify-email' });

  await this.usersRepo.update(userId, { emailVerified: true });
  return { ok: true, emailVerified: true };
}

async forceMfa(actor: { id: string; role: Role; }, userId: string, meta?: any) {
  const user = await this.mustGetUser(userId);
...
async resetPassword(actor: { id: string; role: Role; }, userId: string, meta?: any) {
  const user = await this.mustGetUser(userId);
  assertCanActOnUser({ actorRole: actor.role, targetRole: user.role_key, action: 'reset-password' });

  await this.usersRepo.update(userId, { passwordChangedAt: new Date() });
  return { ok: true };
}
async revokeSessions(actor: { id: string; role: Role; }, userId: string, meta?: any) {
  const user = await this.mustGetUser(userId);
  assertCanActOnUser({ actorRole: actor.role, targetRole: user.role_key, action: 'revoke-sessions' });
